...
class SubmissionStorage:
                        ...
                        for line in f:
                            ...
                            if line:
                                try:
                                    ...
                                    continue
                    logger.info(
                        f"Loaded {len(self._data)} submissions from {DATA_FILE}"
                    )
    ...
    def add_submission(
        self, content: str, ip_address: str = None, user_agent: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_submissions(
        ...
        if start_date:
            filtered = [
                item for item in filtered if item["created_at"][:10] >= start_date
            ]

        if end_date:
            filtered = [
                item for item in filtered if item["created_at"][:10] <= end_date
            ]

        # Keyword search
        if keyword:
            if case_sensitive:
                filtered = [item for item in filtered if keyword in item["content"]]
            else:
...
def add_submission(
    content: str, ip_address: str = None, user_agent: str = None
) -> Dict[str, Any]:
    """Add a new submission."""
...
def get_submissions(
    ...
    return storage.get_submissions(
        page, per_page, start_date, end_date, keyword, case_sensitive
    )
